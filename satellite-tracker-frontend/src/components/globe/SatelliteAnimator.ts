...
export function generateOrbit(
    ...
    for (let i = 0; i < points; i++) {
        ...
        const orbitalRadius =
            altitude_km * (1 + Math.sin(angle) * eccentricity);

        const latitude = Math.sin(angle) * inclination;

        let longitude = (angle * 180) / Math.PI;
...
export class SatelliteAnimator {
    ...
    update(
        ...
        for (const satellite of satellites) {
            ...
            const exactIndex =
                satellite.elapsed_seconds / satellite.step_seconds;
